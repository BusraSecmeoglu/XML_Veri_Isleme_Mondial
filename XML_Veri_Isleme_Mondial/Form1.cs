...
namespace XML_Veri_Isleme_Mondial
{
    ...
    {
        private readonly string _filePath = @"C:\Users\iau\Desktop\XML_Projects-1\Original\XML_Veri_Isleme_Mondial\XML_Veri_Isleme_Mondial\XML_Veri_Isleme_Mondial\mondial-3.0.xml";
        ...
        private void SearchCountry_Click(object sender, EventArgs e)
        {
            ...
            //var country = _countries.First(x => x.DataCode == tbSearchCountry.Text);
            //var countryName = country.CountryName;
            string dataCode = tbSearchCountry.Text;
            if (string.IsNullOrWhiteSpace(dataCode))
                return;
            foreach (var country in _countries)
            {
                if (country.DataCode != null && country.DataCode.Equals(dataCode))
                {
